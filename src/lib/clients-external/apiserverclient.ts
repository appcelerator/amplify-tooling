...
export class ApiServerClient {
  ...
  async bulkCreate(
    ...
    for (const resource of resources) {
      ...
      if (res.data && !res.error) {
        ...
        if (!!res.pending) {
          ...
          if (res.warning) bulkResult.warning?.push(res.data);
          else bulkResult.success.push(res.data);
        }
      } else if (res.error) {
    ...
    for (const p of pendingSubResources) {
      ...
      if (subResResult.data && !subResResult.error) {
        if (p.withWarning) bulkResult.warning?.push(subResResult.data);
        else bulkResult.success.push(subResResult.data);
  ...
  async bulkCreateOrUpdate(
    ...
    for (const resource of resources) {
      ...
      let getResult: ApiServerClientSingleResult | null = resource.name
