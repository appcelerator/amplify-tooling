...
export function assertNodeEngineVersion(pkgJson) {
	...
	const current = process.env.AXWAY_TEST_NODE_VERSION || process.version;
 ...
 * TODO: The use of this can just use Intl
 *
	...
	if (typeof process._getActiveHandles === 'function') {
		for (let handle of process._getActiveHandles()) {
			if (Timer && handle instanceof Timer) {
				...
				handle instanceof EventEmitter &&
				typeof (handle as any).start === 'function' &&
				typeof (handle as any).close === 'function' &&
				FSEvent &&
				(handle as any)._handle instanceof FSEvent
			) {
