...
a SSH terminal, you must set the token store type to "file":
  ...
  List all service accounts:
    ${highlight('axway service-account list')}
  ...
  Change a service account name, description, and role:
    ${highlight('axway service-account update <name/client-id> --name <new_name> --desc <desc> --role <role>')}

  Add a team to an existing service account:
    ${highlight('axway service-account add-team <client-id/name> <team_guid> <team_role>')}

  Remove a team from a service account:
    ${highlight('axway service-account remove-team <client-id/name> <team_guid>')}

  Remove a service account:
    ${highlight('axway service-account remove <client-id/name>')}

  View available team roles:
    ${highlight('axway service-account roles')}`;
		}
	},
	helpExitCode: 2,
