	...
	static override examples = [
		{
			...
			command: '<%= config.bin %> <%= command.id %> <team> --name <new name>'
		},
		{
			description: 'Update the team description',
			command: '<%= config.bin %> <%= command.id %> <team> --desc <new description>'
		},
		{
			description: 'Redefine the team tags',
			command: '<%= config.bin %> <%= command.id %> <team> --tag <tag1> --tag <tag2>'
		}
	];
