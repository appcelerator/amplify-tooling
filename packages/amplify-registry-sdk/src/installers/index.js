...
export class PackageInstaller extends EventEmitter {
	constructor({ name, repository, type, fetchSpec, env, url }) {
		super();
		this.registry = new Registry({ env, url });
		Object.assign(this, { name, repository, type, fetchSpec });
	}

	async start () {
		const pkgInfo = await this.registry.metadata({ name: this.name, repository: this.repository, type: this.type, version: this.fetchSpec });
		if (!pkgInfo) {
