...
describe('axway engage get', () => {
	describe('help', () => {
		it('should output the help screen', async () => {
			const { status, stdout } = await runAxwaySync([ 'engage', 'get', '--help' ]);
	...
	describe('auth required', () => {
		...
		it('should error if not authenticated', async () => {
			...
			const { status, stderr } = await runAxwaySync([ 'engage', 'get', 'environment' ], { env: engageEnv });
	...
	describe('authenticated', () => {
		...
		describe('args validation', () => {
			it('should error if no resource type is provided', async () => {
				const { status, stdout } = await runAxwaySync([ 'engage', 'get' ], { env: engageEnv });
				expect(status).to.equal(1);
			...
			it('should error if an invalid resource type is provided', async () => {
				const { status, stderr } = await runAxwaySync([ 'engage', 'get', 'invalidtype123' ], { env: engageEnv });
				expect(status).to.equal(1);
		...
		describe('get list', () => {
			it('should list environments (management group) as a table', async () => {
				...
				const { status, stdout } = await runAxwaySync([ 'engage', 'get', 'environment' ], { env: engageEnv });
			...
			it('should list webhooks using the "wh" shortName', async () => {
				...
				const { status, stdout } = await runAxwaySync([ 'engage', 'get', 'wh' ], { env: engageEnv });
			...
			it('should list catalog resources (stages) as a table', async () => {
				...
				const { status, stdout } = await runAxwaySync([ 'engage', 'get', 'stage' ], { env: engageEnv });
