...
describe('axway team', () => {
	describe('help', () => {
		...
		it('should output the help screen with color', async () => {
			const { status, stdout } = await runAxwaySync([ 'team' ], { color: true });
		...
		it('should output the help screen using --help flag', async () => {
			const { status, stdout } = await runAxwaySync([ 'team', '--help' ], { color: true });
			expect(stdout.toString()).to.match(renderRegexFromFile('help/help-with-color', {}, { color: true }));
			expect(status).to.equal(0);
		});
	});
	...
	describe('user', () => {
		...
		describe('add', () => {
			...
			it('should error if not logged in', async function () {
				...
				const { status, stderr } = await runAxwaySync([ 'team', 'user', 'add', 'foo', 'bar' ]);
			...
			it('should add a user to a team by email address', async function () {
				...
				let { status, stdout } = await runAxwaySync([ 'team', 'user', 'add', 'a team', 'test2@domain.com', '--role', 'developer' ]);
				expect(stdout).to.match(renderRegexFromFile('user/add/success'));
				expect(status).to.equal(0);

				({ status, stdout } = await runAxwaySync([ 'team', 'user', 'list', 'a team' ]));
