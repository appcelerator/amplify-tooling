...
import tmp from 'tmp';
import { Telemetry } from '../../../dist/lib/amplify-sdk/index.js';
import { readJsonSync, writeJsonSync } from '../../../dist/lib/fs.js';
...
describe('Telemetry', () => {
	describe('Error handling', () => {
		...
		it('should error if url is invalid', () => {
			expect(() => {
				new Telemetry({ appGuid: 'foo', appVersion: '1.0.0', cacheDir: 'bar', environment: 'test' });
			}).to.throw(TypeError, 'Expected telemetry URL to be a non-empty string');

			expect(() => {
				new Telemetry({ appGuid: 'foo', appVersion: '1.0.0', cacheDir: 'bar', environment: 'test', url: '' });
			}).to.throw(TypeError, 'Expected telemetry URL to be a non-empty string');

			expect(() => {
				new Telemetry({ appGuid: 'foo', appVersion: '1.0.0', cacheDir: 'bar', environment: 'test', url: 123 });
			}).to.throw(TypeError, 'Expected telemetry URL to be a non-empty string');
		});
	...
	describe('Send events', () => {
		...
		it('should add many events and send 2 batches', async function () {
			...
			let events = this.getTelemetryEvents()[0];
		...
		it('should end an old session and start a new one', async function () {
			...
			writeJsonSync(path.join(appDir, '.sid'), json);
			...
			expect(this.getTelemetryEvents()).to.have.lengthOf(2);
			const events = this.getTelemetryEvents()[1];
...
function createTelemetry(opts = {}) {
	const telemetry = new Telemetry({
		...
		cacheDir,
		environment: 'test',
		url: 'http://127.0.0.1:8666/api/v1/analytics',
		...opts
	});
