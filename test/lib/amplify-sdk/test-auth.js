...
describe('Auth', () => {
	...
	describe('Server Info', () => {
		...
		it('should fetch server info', async function () {
			...
			const info = await auth.serverInfo({ url: 'http://127.0.0.1:8555/auth/realms/test_realm/.well-known/openid-configuration' });
			expect(info).to.deep.equal(serverInfo);
		});

		it('should throw error if server returns error', async function () {
			nock('http://127.0.0.1:8555')
				.get('/auth/realms/test_realm/.well-known/openid-configuration')
				.once()
